...
class Mpesa
{
    ...
    public function encryptKey($key): string
    {
        $pKey = openssl_pkey_get_public(
            sprintf('-----BEGIN PUBLIC KEY-----%s-----END PUBLIC KEY-----', $this->options['public_key'])
        );
        if ($pKey === false) {
            throw new \RuntimeException('Unable to load public key');
        }
        // openssl_public_encrypt($key, $encrypted, $pKey);
        if (openssl_public_encrypt($key, $encrypted, $pKey, OPENSSL_PKCS1_OAEP_PADDING) === false) {
    ...
    public function getSessionToken($session = null)
    {
        ...
        if ($resSession['output_ResponseCode'] == 'INS-0') {
            ...
            throw new \RuntimeException($resSession['output_ResponseDesc'] ?? "Error Processing Request", $resSession['output_ResponseCode'] ?? 0);
        }
    }
